...
def generate_hash(message: bytearray, printmessage=False) -> bytearray:
    ...
    for message_block in blocks:
        ...
        for t in range(64):
            ...
            f = e
            if(printmessage):
                print(((d+t1) % 2**32).to_bytes(4,'big'))
            e = (d + t1) % 2**32
            d = c
            c = b
            b = a
            if(printmessage):
                print(((t1 + t2) % 2**32).to_bytes(4,'big'))
            a = (t1 + t2) % 2**32

            if(printmessage):
                print(f"{t} " + "#"*25)
                print(f"T1 : {t1.to_bytes(4,'big')}")
                print(f"T2 : {t2.to_bytes(4,'big')}")
                print(f"working : {[a.to_bytes(4,'big'),b.to_bytes(4,'big'),c.to_bytes(4,'big'),d.to_bytes(4,'big'),e.to_bytes(4,'big'),f.to_bytes(4,'big'),g.to_bytes(4,'big'),h.to_bytes(4,'big')]}")
